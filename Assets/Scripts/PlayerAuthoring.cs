...
namespace DefaultNamespace
{
        ...
        public class Baker : Baker<PlayerAuthoring>
        {
            public override void Bake(PlayerAuthoring authoring)
            {
                ...
                AddComponent(entity, new PlayerAttackData()
                {
                    ...
                    DectionSize = new float3(authoring.DectionSize),
                ...
                SetComponentEnabled<UpdateGemUIFlag>(entity, false);
    ...
    public partial struct CameraInitializationSystem : ISystem
    {
        ...
        {
            if (CameraTargetSingleten.Instance == null) return;
            var cameraTargetTransform = CameraTargetSingleten.Instance.transform;

            var ecb = new EntityCommandBuffer(state.WorldUpdateAllocator);
            foreach (var (cameraTarget, entity) in SystemAPI.Query<RefRW<CameraTarget>>()
                         .WithAll<PlayerTag, InitializeCameraTargetTag>().WithEntityAccess())
            {
    ...
    public partial struct PlayerAttackSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
                         ...
                         .Query<RefRW<PlayerCooldownExpirationTimestamp>, PlayerAttackData, LocalTransform>())
            {
                ...
                var minDetectPosition = spawnPosition - attackData.DectionSize;
                var maxDetectPosition = spawnPosition + attackData.DectionSize;
                ...
                physicsWorldSingleton.OverlapAabb(aabbInput, ref overlapHits);
    ...
    public partial struct UpdateGemUISystem : ISystem
    {
        public void OnUpdate(ref SystemState state)
        {
            foreach (var (gemCount, shouldUpdateUI) in SystemAPI
                         .Query<GemsCollectedCount, EnabledRefRW<UpdateGemUIFlag>>())
            {
    ...
    public partial struct PlayerWorldUISystem : ISystem
    {
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (worldUI, entity) in SystemAPI.Query<PlayerWorldUI>().WithNone<LocalTransform>()
                         .WithEntityAccess())
            {
